...
import com.mcd.gdw.test.daas.mapreduce.FindFileIdsMapper;
import com.mcd.gdw.test.daas.mapreduce.FindFileIdsReducer;

public class FindFileIds extends Configured implements Tool {

	//public static final String FIND_LIST = "FindDwFileId.txt";
	public static final int POS_FILE_TYPE = 0;
	public static final int POS_FILE_NAME = 1;

	public static final String FIND_LIST = "FindLocDates.txt";
	//public static final int POS_FILE_TERR_CD = 0;
	//public static final int POS_FILE_LCAT = 1;
	//public static final int POS_FILE_DATE = 2;
	...
	public static void main(String[] args) throws Exception {
		...
		int retval = ToolRunner.run(new Configuration(),new FindFileIds(), args);
	...
	public int run(String[] argsall) throws Exception {
		...
		if ( daasConfig.configValid() ) {
			...
			cachePath = new Path("/user/mc05112/" + FIND_LIST);
			
			fileSystem = FileSystem.get(getConf());
			
			runFindFileIds(daasConfig,fileType,getConf(),terrDate,false);
	...
	private void runFindFileIds(DaaSConfig daasConfig
                               ,String fileType
		...
		try {
			...
			job = Job.getInstance(hdfsConfig, "Find DW FILE IDs");
			...
			outPath= new Path(daasConfig.hdfsRoot() + Path.SEPARATOR + daasConfig.hdfsWorkSubDir() + Path.SEPARATOR + "FindIds");
			...
			job.setJarByClass(FindFileIds.class);
			job.setMapperClass(FindFileIdsMapper.class);
			job.setReducerClass(FindFileIdsReducer.class);
