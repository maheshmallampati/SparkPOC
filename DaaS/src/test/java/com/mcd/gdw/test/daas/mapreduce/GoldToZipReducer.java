...
public class GoldToZipReducer extends Reducer<Text, Text, Text, Text> {
	...
	protected void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
		...
		zipOutputFileName = "POS_XML_US_" + keyParts[0] + "_" + keyParts[1] + "." + timestamp + ".zip"; 
		...
		while ( nextValue.hasNext() ) {
			...
			newKey.clear();
			newKey.set("POS_" + fileSubType + "_US_" + keyParts[0] + "_" + keyParts[1] + "." + timestamp + ".xml");
			
			mos.write("ZIPOUTPUT", newKey, newValue,  zipOutputFileName);

		}
		
		context.getCounter("DaaS Counters", "Zip Files Created").increment(1);
