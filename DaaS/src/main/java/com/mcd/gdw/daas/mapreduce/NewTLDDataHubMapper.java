...
public class NewTLDDataHubMapper extends Mapper<LongWritable, Text, Text, Text> {
...
//	private HashMap<String,IncludeListDates> includeListMap = new HashMap<String,IncludeListDates>();
			...
			for (int idxEvent=0; idxEvent < nlEvent.getLength(); idxEvent++ ) {
				...
				if ( nlEvent.item(idxEvent).getNodeType() == Node.ELEMENT_NODE ) {  
					...
					if ( eleEvent.getNodeName().equals("Event") &&
						...
						processTrx(eleEvent,eleEvent.getChildNodes(),context);
	...
	private void processTrx(Element eleEvent,NodeList nlTrxSale
			                   ,Context context) {
			...
			if (nlOrder != null && nlOrder.getLength() > 0 ) {
				for (int idxOrder=0; idxOrder < nlOrder.getLength(); idxOrder++ ) {
					if ( nlOrder.item(idxOrder).getNodeType() == Node.ELEMENT_NODE ) {
						...
						orderTs   = getValue(eleOrder, "Timestamp");
...
//						context.getCounter("COUNT","POS_TRN_ITM_UNIQUE").increment(1);
						...
						orderInfo.append(orderTs).append(SEPARATOR_CHARACTER);
						...
						orderInfo.append(posTotNetTrnAm).append(SEPARATOR_CHARACTER);//@todo
						orderInfo.append("posTotGrssTrnAm").append(SEPARATOR_CHARACTER);//@todo
						orderInfo.append(posTotNprdNetTrnAm).append(SEPARATOR_CHARACTER);//@todo
