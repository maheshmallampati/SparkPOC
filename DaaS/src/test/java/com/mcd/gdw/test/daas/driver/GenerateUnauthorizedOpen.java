...
import com.mcd.gdw.test.daas.mapreduce.UnauthorizedDataMapper;
import com.mcd.gdw.test.daas.mapreduce.UnauthorizedDataReducer;

public class GenerateUnauthorizedOpen extends Configured implements Tool {
	...
	public static String fileNameUnauthorizedOpen="UnauthorizedOpen";
	...
	public static void main(String[] args) throws Exception {
		...
		int retval = ToolRunner.run(new Configuration(),new GenerateUnauthorizedOpen(), args);
	...
	private void runMR(DaaSConfig daasConfig
		...
		try {
			...
			job = new Job(hdfsConfig, "Generate Unauthorized Data");
			
			if ( daasConfig.displayMsgs() ) {
				System.out.println("\nGenerate Unauthorized Data\n");
			}
			Path outPath=null;

			outPath= new Path(daasConfig.hdfsRoot() + Path.SEPARATOR + daasConfig.hdfsWorkSubDir() + Path.SEPARATOR + "UnauthorizedData");
			...
			job.setJarByClass(GenerateUnauthorizedOpen.class);
			job.setMapperClass(UnauthorizedDataMapper.class);
			job.setReducerClass(UnauthorizedDataReducer.class);
			...
			MultipleOutputs.addNamedOutput(job,HDFSUtil.replaceMultiOutSpecialChars(GenerateUnauthorizedOpen.fileNameUnauthorizedOpen), TextOutputFormat.class, Text.class,
					Text.class);
