...
public class GenerateTLDDataHubFormat extends Configured implements Tool {
...
//	FsPermission newFilePremission;
	...
	public static void main(String[] args) throws Exception {
		...
		int retval = ToolRunner.run(hdfsConfig,new GenerateTLDDataHubFormat(), args);
...
//					abac.closeJobGroup(jobGrpId, DaaSConstants.JOB_SUCCESSFUL_ID, DaaSConstants.JOB_SUCCESSFUL_CD);
			...
			for (Path addPath : requestedPaths ) {
				...
				dt  = temp[temp.length-1].subSequence(0,4)+"RxD045"+temp[temp.length-1].substring(4,6)+"RxD045"+temp[temp.length-1].substring(6, 8);
...
//			job.addCacheFile(new URI(locationList.toString() + "#" + locationList.getName()));
			...
			job.setJarByClass(GenerateTLDDataHubFormat.class);
			job.setMapperClass(TLDDataHubMapper.class);
			job.setReducerClass(TLDDataHubReducer.class);
