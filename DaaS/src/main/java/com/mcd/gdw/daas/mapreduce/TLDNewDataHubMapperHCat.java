...
/** ##########################################################################
# File Name:   TLDNewDataHubMapperHCat.java 
# Project Name: 
# Written By:           
# Date Created:        
#
...
#
#Change Log: 
#         Added a method isValidXML(String xmlText, Context context) at line 2537
#                    Date Modified:        2016/02/15
#                    Developer Name:       Sarath Ikkurthy
#                    Description: This method validates if the input xml is valid or not and if not valid then it should be excluded from parsing
#
#
########################################################################## */
...
public class TLDNewDataHubMapperHCat extends Mapper<LongWritable, Text, Text, Text> {
...
//		public class TLDNewDataHubMapperHCat extends Mapper<LongWritable, Text, NullWritable, OrcFile> {
		...
		private static final String ORDER_ITEM_GRILLMODIFER		= "grillModifier";
		...
		public static final int US_TERR_CD = 840;
		public static final int AUS_TERR_CD = 36;
		public static final String SPLSTR_TAP ="(t)";
		public static final String SPLSTR_SWIPE ="(s)";
		public static final String SPLSTR_PIN ="(p)";
		public static final String SPLSTR_CHIP ="(c)";
		public static final String SPLSTR_ACCOUNT_TYPE = "ACCOUNT TYPE      CREDIT";
...
//	private MultipleOutputs<Text, Text> multipleOutputs;
	private String INVALID_XML_LOG="InvalidXMLLog";
		...
		try {
		    ...
		    if ( distPaths != null && distPaths.length > 0 )  {
		    	...
		    	for ( int i=0; i<distPaths.length; i++ ) {
				      ...
				      }else if(distPaths[i].toString().contains("MenuPriceBasis.psv")){
				    	  menuPriceBasisMap = new HashMap<Integer,String>();
				    	  br  = new BufferedReader(new FileReader("./" + distPathParts[1]));
				    	  addKeyValuestoMap2(br);
				      }
...
//			System.exit(8);
	 ...
	 private void addKeyValuestoMap2(BufferedReader br){
		  ...
		  try { 
		      ...
		      while ( (line = br.readLine() )!= null) {
		        ...
		        if (flds.length == 2) {
		        	System.out.println("Fields are"+flds[0] +"and"+flds[1]);
		        	menuPriceBasisMap.put(new Integer(flds[0].trim()), flds[1].trim());
		            //System.out.println("Inside Add Key");
		        }   
		      }
		    } catch (IOException e) { 
		      e.printStackTrace();
	...
	public void map(LongWritable key, Text value,Context context) throws IOException, InterruptedException {
		...
		try {
			...
			//To check the xml is valid if not we should skip it
			includeFile=isValidXML(parts[DaaSConstants.XML_REC_XML_TEXT_POS],context);
...
//				
//				if(! terrCd.equalsIgnoreCase("840") || !posBusnDt.equalsIgnoreCase("2015-04-08") || !(lgcyLclRfrDefCd.equalsIgnoreCase("14296"))){
...
//						context.write(NullWritable.get(), uniqueTerrCdBusnDtStoreIdVal);
						context.write(terrcdbusndtstoreid, uniqueTerrCdBusnDtStoreIdVal);
...
//						
						...
						//@mc42478: Fix for Number format exception
						try{
							totalTaxbg  = new BigDecimal(posTotTaxAm);
						}
						catch(Exception ex){
							totalTaxbg  = new BigDecimal("0");
						}
...
//						context.getCounter("COUNT","POS_TRN_ITM_UNIQUE").increment(1);
						...
						String menuPriceBasis = menuPriceBasisMap.get(new Integer(terrCd));
...
//						            tenderInfo.append(faceValue).append(SEPARATOR_CHARACTER);//FaceValue
						            ...
						            if(StringUtils.isNotBlank(cashlessData) && Integer.parseInt(terrCd) == US_TERR_CD){
...
//						            	System.out.println(" cashlessDataArr -----  "+ cashlessDataArr[2] + "XXXX"+cashlessData + " ------");
						            ...
						            else if( Integer.parseInt(terrCd) == AUS_TERR_CD && StringUtils.isNotBlank(cashlessData)){
						            	cashlessDataArr = cashlessData.split("@#",-1)[0].split("\\\\n");
						            	if(cashlessData.length() > 10 ){
						        			int cardIndex=0;
						        			if(cashlessData.contains(SPLSTR_TAP)){
						        				 cardIndex =  cashlessData.indexOf(SPLSTR_TAP);
						        			}
						        			else if( cashlessData.contains(SPLSTR_SWIPE)){
						        				cardIndex =  cashlessData.indexOf(SPLSTR_SWIPE);
						        			}
						        			else if (cashlessData.contains(SPLSTR_PIN)){
						        				cardIndex =  cashlessData.indexOf(SPLSTR_PIN);
						        			}
						        			else if (cashlessData.contains(SPLSTR_CHIP)){
						        				cardIndex =  cashlessData.indexOf(SPLSTR_CHIP);
						        			}
						        			if (cardIndex > 0 && cashlessData.contains(SPLSTR_ACCOUNT_TYPE))
						        			tenderInfo.append(cashlessData.substring(cardIndex-4, cardIndex)).append(SEPARATOR_CHARACTER);//1_POS_CSHL_CARD_NU
						        			else 
						        				tenderInfo.append(StringUtils.EMPTY).append(SEPARATOR_CHARACTER);//1_POS_CSHL_CARD_NU
						        		}
						            	tenderInfo.append(StringUtils.EMPTY).append(SEPARATOR_CHARACTER);//1_POS_CSHL_ATHZ_CD
						            	if(cashlessDataArr.length > 10)
						            		tenderInfo.append(tenderAmount).append(SEPARATOR_CHARACTER);//1_POS_CSHC_PYMT_AM
						            	else
						            	tenderInfo.append(StringUtils.EMPTY).append(SEPARATOR_CHARACTER);//1_POS_CSHC_PYMT_AM
						            ...
						            else{// for China and Singapore and for the fields which has no cashless data tag
						            	tenderInfo.append(StringUtils.EMPTY).append(SEPARATOR_CHARACTER);//1_POS_CSHL_CARD_NU
						            	tenderInfo.append(StringUtils.EMPTY).append(SEPARATOR_CHARACTER);//1_POS_CSHL_ATHZ_CD
						            	tenderInfo.append(StringUtils.EMPTY).append(SEPARATOR_CHARACTER);//1_POS_CSHC_PYMT_AM
						            	tenderInfo.append(StringUtils.EMPTY).append(SEPARATOR_CHARACTER);//1_POS_CSHL_TOKN_CD
	...
	private void processItem(NodeList nlItem
		...
		String posItmTaxBaseAm = StringUtils.EMPTY;
		String posItmTaxAm = StringUtils.EMPTY;
		
		
		BigDecimal itemTaxRatebg = null;
		BigDecimal posItmTaxBaseAmbg = null;
		BigDecimal posItmTaxAmbg = null;
					...
					if ( nlItem.item(idxItem).getNodeType() == Node.ELEMENT_NODE ) {
						...
						if ( eleItem.getNodeName().equals("Item") ) {
							...
							if(itemTaxChain != null){
								...
								posItmTaxBaseAm= ""+Double.parseDouble(itemTaxChain.getAttribute("baseAmount"));//baseAmount
								posItmTaxAm = ""+Double.parseDouble(itemTaxChain.getAttribute("amount"));//Amount
								posItmTaxBaseAmbg =new BigDecimal(posItmTaxBaseAm);
								posItmTaxAmbg = new BigDecimal(posItmTaxAm);
							...
							try{
								...
								posItmActUntPrcAmbg  = new BigDecimal("0");
							}
							...
							try{
								posItmTaxRateAmbg = new BigDecimal(posItmTaxRateAm);
							}catch(Exception ex){
								posItmTaxRateAmbg =  new BigDecimal("0");
							}
...
//							itemInfo.append(posItmActTaxAm).append(SEPARATOR_CHARACTER);//totalTax@tocheck repeated
							...
							if(menuPriceBasis.equalsIgnoreCase("N")){
								...
								if(posItmPrmoQt != null && Integer.parseInt(posItmPrmoQt) > 0){
									POS_ITM_NET_PRMO_AMbg = (posItmNetUntPrcB4PrmoAmbg.subtract(posItmActUntPrcAmbg)).multiply((new BigDecimal(1)).subtract(itemTaxRatebg));
								}else{
									POS_ITM_NET_PRMO_AMbg = BigDecimal.ZERO; 
								}
							}
							...
							if(menuPriceBasis.equalsIgnoreCase("N")){
								...
								if(posItmPrmoQt != null && Integer.parseInt(posItmPrmoQt) > 0){
									POS_ITM_GRSS_PRMO_AMbg = posItmNetUntPrcB4PrmoAmbg.subtract(posItmActUntPrcAmbg);
								}
								else{
									POS_ITM_GRSS_PRMO_AMbg = BigDecimal.ZERO;
								}
							...
							if(menuPriceBasis.equalsIgnoreCase("N")){
								...
								if(itemPromotionAppliedInfoArr != null && "PRICE".equalsIgnoreCase(itemPromotionAppliedInfoArr[5])){
									POS_ITM_NET_PRC_DISC_AMbg = (itemPromotionAppliedOriginalPricebg.multiply(posItmTotQtbg).subtract(posItmActUntPrcAmbg)).multiply((new BigDecimal(1)).subtract(itemTaxRatebg));
									itemInfo.append(POS_ITM_NET_PRC_DISC_AMbg.doubleValue()).append(SEPARATOR_CHARACTER);
								}
								else{
									POS_ITM_NET_PRC_DISC_AMbg =(posItmNetUntPrcB4DiscAmbg.subtract(posItmActUntPrcAmbg)).multiply((new BigDecimal(1)).subtract(itemTaxRatebg));
									itemInfo.append(POS_ITM_NET_PRC_DISC_AMbg.doubleValue()).append(SEPARATOR_CHARACTER);
								}
							...
							if(menuPriceBasis.equalsIgnoreCase("N")){
								if(itemPromotionAppliedInfoArr != null && "PRICE".equalsIgnoreCase(itemPromotionAppliedInfoArr[5])){
									POS_ITM_GRSS_PRC_DISC_AMbg = itemPromotionAppliedOriginalPricebg.multiply(posItmTotQtbg).subtract(posItmActUntPrcAmbg).multiply(itemTaxRatebg.add(new BigDecimal(1)));
								...
								if(itemPromotionAppliedInfoArr != null && "PRICE".equalsIgnoreCase(itemPromotionAppliedInfoArr[5])){
									...
									POS_ITM_GRSS_PRC_DISC_AMbg = posItmNetUntPrcB4DiscAmbg.subtract(posItmActUntPrcAmbg);
									itemInfo.append(POS_ITM_GRSS_PRC_DISC_AMbg.doubleValue()).append(SEPARATOR_CHARACTER);
								}
							...
							if(menuPriceBasis.equalsIgnoreCase("N")){
								if(posItmNetUntPrcB4DiscAmbg.compareTo(BigDecimal.ZERO ) > 0){
									if("PRICE".equalsIgnoreCase(itemPromotionAppliedInfoArr[5])){
										...
										itemInfo.append(posItmTaxRateAmbg.doubleValue()).append(SEPARATOR_CHARACTER);
									}
								}else{
								...
								if(posItmNetUntPrcB4DiscAmbg.compareTo(BigDecimal.ZERO ) > 0){
									if("PRICE".equalsIgnoreCase(itemPromotionAppliedInfoArr[5])){
										itemInfo.append((itemPromotionAppliedOriginalPricebg.multiply((new BigDecimal(1)).subtract(itemTaxRatebg))).doubleValue()).append(SEPARATOR_CHARACTER);
									}else{
										itemInfo.append(posItmTaxRateAmbg.multiply((new BigDecimal(1)).subtract(itemTaxRatebg))).append(SEPARATOR_CHARACTER);
									}
								}else{
									itemInfo.append((posItmNetUntPrcB4PrmoAmbg.multiply((new BigDecimal(1)).subtract(itemTaxRatebg))).doubleValue()).append(SEPARATOR_CHARACTER);
								}
							...
							if(menuPriceBasis.equalsIgnoreCase("N")){
								if(posItmNetUntPrcB4DiscAmbg.compareTo(BigDecimal.ZERO ) > 0){
									if("PRICE".equalsIgnoreCase(itemPromotionAppliedInfoArr[5])){
										itemInfo.append(itemPromotionAppliedOriginalPricebg.multiply(itemTaxRatebg.add(new BigDecimal(1))).doubleValue()).append(SEPARATOR_CHARACTER);
									}else{
										itemInfo.append(posItmTaxRateAmbg.multiply(itemTaxRatebg.add(new BigDecimal(1))).doubleValue()).append(SEPARATOR_CHARACTER);
									}
								}else{
									itemInfo.append(posItmNetUntPrcB4PrmoAmbg.multiply(itemTaxRatebg.add(new BigDecimal(1))).doubleValue()).append(SEPARATOR_CHARACTER);
								}
							}else{// gross markets
								if(posItmNetUntPrcB4DiscAmbg.compareTo(BigDecimal.ZERO ) > 0){
									if("PRICE".equalsIgnoreCase(itemPromotionAppliedInfoArr[5])){
...
//							itemInfo.append("POS_ITM_ORGN_GRSS_UNT_PRC_AM").append(SEPARATOR_CHARACTER);
							...
							itemInfo.append(posItmTaxAmbg).append(SEPARATOR_CHARACTER);//POS_ITM_TAX_RATE_AM
							itemInfo.append(posItmTaxBaseAmbg).append(SEPARATOR_CHARACTER);//POS_ITM_TAX_BAS_AM
...
//		mos.close();
	...
	/**
	 * This method validates if the input xml is valid or not and if not valid then it should 
	...
	private boolean isValidXML(String xmlText, Context context) throws Exception {

		boolean include = false;
		Text outputTxt = new Text();
		StringBuffer outputStrBuf = new StringBuffer();
		
		String terrCode = String.format("%03d", Integer.parseInt(parts[DaaSConstants.XML_REC_TERR_CD_POS]));
		String posBusinessDt = formatDateAsTsDtOnly(parts[DaaSConstants.XML_REC_POS_BUSN_DT_POS]);
		//String mcdGBALCATID = parts[DaaSConstants.XML_REC_MCD_GBAL_LCAT_ID_NU_POS];
		String legacyLclReferDefCD = parts[DaaSConstants.XML_REC_LGCY_LCL_RFR_DEF_CD_POS];
		...
		try {
			...
			System.err.println("Error occured in TLDNewDataHubMapperHCat.isValidXML:");
			ex.printStackTrace(System.err);
		} finally {
			xmlText = null;
			doc = null;
		...
		if(!include){
			...
			outputStrBuf.append(legacyLclReferDefCD).append("\t").append(terrCode)
			.append("\t").append(posBusinessDt);
			
			outputTxt.set(outputStrBuf.toString());
			System.err.println("Invalid XML found "+outputStrBuf.toString());
			//multipleOutputs.write(INVALID_XML_LOG,NullWritable.get(), outputTxt);
		}
		return include;
