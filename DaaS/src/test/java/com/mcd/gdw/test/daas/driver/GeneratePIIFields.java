...
import com.mcd.gdw.test.daas.mapreduce.PIIFieldsMapper;
import com.mcd.gdw.test.daas.mapreduce.PIIFieldsReducer;
...
public class GeneratePIIFields extends Configured implements Tool {
	...
	private final static String PII_EXTRACT_JOBGROUP_NAME = "GDW Metric Extract";
	private final static String PII_EXTRACT_HISTORY_JOBGROUP_NAME = "GDW Metric Extract";
	...
	public static final String PII_METRICS_FILE = "PIIDetails";
	public static final String PII_FILE_STORE_COUNTS="PIIDetailCounts";
	...
	public static void main(String[] args) throws Exception {
		...
		ToolRunner.run(new Configuration(), new GeneratePIIFields(), args);
	}
	...
	private void runMrGDWWorkLayerMetricsExtract(DaaSConfig daasConfig, String fileType,
		...
		try {
			...
			if (!terrDate.toUpperCase().startsWith("WORK")) {
				jobTitle = PII_EXTRACT_HISTORY_JOBGROUP_NAME;
			} else {
				jobTitle = PII_EXTRACT_JOBGROUP_NAME;
			}
			...
			job.setJarByClass(GeneratePIIFields.class);
			job.setMapperClass(PIIFieldsMapper.class);
			job.setReducerClass(PIIFieldsReducer.class);
			...
			MultipleOutputs.addNamedOutput(job,HDFSUtil.replaceMultiOutSpecialChars(PII_METRICS_FILE), TextOutputFormat.class, Text.class,
			Text.class);
			MultipleOutputs.addNamedOutput(job,HDFSUtil.replaceMultiOutSpecialChars(PII_FILE_STORE_COUNTS), TextOutputFormat.class, Text.class,
					Text.class);
