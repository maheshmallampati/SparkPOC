...
import com.mcd.gdw.test.daas.mapreduce.SampleReportFormatMapper;
import com.mcd.gdw.test.daas.mapreduce.SampleReportFormatReducer;

public class GenerateSampleReport extends Configured implements Tool {

	public final static String SEPARATOR_CHARACTER = "\t";
	public final static String REC_POSTRN          = "TRN";
	public final static String REC_POSTRNOFFR      = "OFR";
	public final static String REC_POSTRNITM       = "ITM";
	public final static String REC_POSTRNITMOFFR   = "IOF";
	
	public final static String NULL_SUB_CHARACTER  = "daas.job.null_sub_character";
	public final static String NULL_SUB_CHARACTER_BLANK_VALUE = "~BLANK_VALUE~";
	...
	private static final String JOB_GRP_DESC = "Generate Sample Extracts";
	...
	public static void main(String[] args) throws Exception {
		...
		int retval = ToolRunner.run(hdfsConfig,new GenerateSampleReport(), args);
	...
	public int run(String[] args) throws Exception {
		...
		String fileType = "";
		String terrDate = "";
		String subNull = NULL_SUB_CHARACTER_BLANK_VALUE;
		...
		for ( int idx=0; idx < args.length; idx++ ) {
			...
			if ( args[idx].equals("-n") && (idx+1) < args.length ) {
				subNull = args[idx+1];
			}
		...
		if ( helpRequest ) {
			System.out.println("Usage: GenerateSampleReport -c config.xml -t filetype -d territoryDateParms ");
		...
		if ( configXmlFile.length() == 0 || fileType.length() == 0 || terrDate.length() == 0 ) {
			...
			System.err.println("Usage: GenerateSampleReport -c config.xml -t filetype -d territoryDateParms");
		...
		if ( daasConfig.configValid() ) {
			...
			runJob(daasConfig,fileType,terrDate,subNull,compressOut);
	...
	private void runJob(DaaSConfig daasConfig
		...
		try {
			...
			baseOutputPath = new Path("/daastest" + Path.SEPARATOR + daasConfig.hdfsWorkSubDir() + Path.SEPARATOR + "SampleOutput");
			...
			hdfsConfig.set(NULL_SUB_CHARACTER, subNull);
			...
			job.setJarByClass(GenerateSampleReport.class);
			job.setMapperClass(SampleReportFormatMapper.class);
			job.setReducerClass(SampleReportFormatReducer.class);
			...
			Path posTrn = new Path(baseOutputPath.toString() + Path.SEPARATOR + "POS_TRN" );
			Path posTrnOffer = new Path(baseOutputPath.toString() + Path.SEPARATOR + "POS_TRN_OFFR" );
			Path posTrnItem = new Path(baseOutputPath.toString() + Path.SEPARATOR + "POS_TRN_ITM" );
			Path posTrnItemOffer = new Path(baseOutputPath.toString() + Path.SEPARATOR + "POS_TRN_ITM_OFFR" );
			
			Path newPath;
			
			HDFSUtil.createHdfsSubDirIfNecessary(fileSystem, posTrn, daasConfig.displayMsgs());
			HDFSUtil.createHdfsSubDirIfNecessary(fileSystem, posTrnOffer, daasConfig.displayMsgs());
			HDFSUtil.createHdfsSubDirIfNecessary(fileSystem, posTrnItem, daasConfig.displayMsgs());
			HDFSUtil.createHdfsSubDirIfNecessary(fileSystem, posTrnItemOffer, daasConfig.displayMsgs());
			...
			if ( status != null ) {
				for (int idx=0; idx < status.length; idx++ ) {
					fileName = status[idx].getPath().getName().toString();
					
					if ( fileName.contains("POSTRN-") ) {
						newPath = new Path(posTrn.toString() + Path.SEPARATOR + fileName);
					} else if ( fileName.contains("POSTRNOFFR-") ) {
						newPath = new Path(posTrnOffer.toString() + Path.SEPARATOR + fileName);
					} else if ( fileName.contains("POSTRNITM-") ) {
						newPath = new Path(posTrnItem.toString() + Path.SEPARATOR + fileName);
					} else if ( fileName.contains("POSTRNITMOFFR-") ) {
						newPath = new Path(posTrnItemOffer.toString() + Path.SEPARATOR + fileName);
					} else {
						newPath = null;
					}
					
					if ( newPath != null ) { 
						System.out.print(status[idx].getPath().toString() + " ");
			...
			System.err.println("Error occured in GenerateSampleReport.runJob:");
	...
	private ArrayList<Path> getVaildFilePaths(DaaSConfig daasConfig
		...
		try {
			...
			System.err.println("Error occured in GenerateSampleReport.getVaildFilePaths:");
			System.err.println(ex.toString());
			ex.printStackTrace();
			System.exit(8);
		}
