...
import com.mcd.gdw.test.daas.mapreduce.USTLDReformatMapper;
import com.mcd.gdw.test.daas.mapreduce.USTLDReformatReducer;

public class USTLDReformat extends Configured implements Tool {
	...
	public static void main(String[] args) throws Exception {
		...
		int retval = ToolRunner.run(hdfsConfig,new USTLDReformat(), args);
	...
	public int run(String[] args) throws Exception {
		...
		if ( helpRequest ) {
			System.out.println("Usage: USTLDReformat -c config.xml -t filetype -d territoryDateParms ");
	...
	private void runJob(DaaSConfig daasConfig
		...
		try {
			...
			baseOutputPath = new Path(daasConfig.hdfsRoot() + Path.SEPARATOR + daasConfig.hdfsWorkSubDir() + Path.SEPARATOR + "USTLD");
			...
			job = Job.getInstance(hdfsConfig, "Extacting US TLD Format");
			...
			job.setJarByClass(USTLDReformat.class);
			job.setMapperClass(USTLDReformatMapper.class);
			job.setReducerClass(USTLDReformatReducer.class);
	...
	private ArrayList<Path> getVaildFilePaths(DaaSConfig daasConfig
		...
		try {
			...
			Path[] requestPaths = HDFSUtil.requestedArgsPaths(fileSystem, daasConfig, requestedTerrDateParms, "STLD", "Product-Db");
