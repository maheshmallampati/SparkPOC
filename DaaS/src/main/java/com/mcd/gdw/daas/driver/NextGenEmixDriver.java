 ...
 * 
 * Driver class for NextGen EMIX to generate daily, hourly and pmix sales for GDW.
 * The job uses NextGenEmixStldMapper, NextGenEmixMenuItemMapper and NextGenEmixReducer for MapReduce implementation.
 * 
 */
public class NextGenEmixDriver extends Configured implements Tool {
	...
	public static final String GLMA_Dly_Sls = "GLMADlySls";
	public static final String HAVI_Dly_Sls = "HAVIDlySls";
	public static final String GLMA_Hrly_Sls = "GLMAHrlySls";
	public static final String HAVI_Hrly_Sls = "HAVIHrlySls";
	public static final String GLMA_Dly_Pmix = "GLMADlyPmix";
	public static final String HAVI_Dly_Pmix = "HAVIDlyPmix";
	...
	public static final Logger LOGGER = Logger.getLogger(NextGenEmixDriver.class.getName());
	...
	/**
	 * Main Method for NextGenEMIXDriver.
	 * @param args
	 * @throws Exception
	 */
	public static void main(String[] args) throws Exception {
		...
		fileHandler  = new FileHandler("./NextgenEmix.log");
		...
		int status = ToolRunner.run(hdfsConfig, new NextGenEmixDriver(), args);
		System.exit(status);
	}
	
	/**
	 * Run Method for Next Gen EMIX.
	...
	public int run(String[] args) throws Exception {
		...
		try {
			for (int idx = 0; idx < args.length; idx++) {
				...
				// Actual parameter used in production to accept a date range or work layer. (Eg: -d 840:2015-09-20:2015-09-21)
			...
			// Get valid input files from Gold, Work or Custom location based on input parameter -d 
			...
			job.setJarByClass(com.mcd.gdw.daas.driver.NextGenEmixDriver.class);
			
			// Add lookup files to distributed cache.
			...
			job.setMapperClass(com.mcd.gdw.daas.mapreduce.NextGenEmixStldMapper.class);
			job.setMapperClass(com.mcd.gdw.daas.mapreduce.NextGenEmixMenuItemMapper.class);
			job.setReducerClass(com.mcd.gdw.daas.mapreduce.NextGenEmixReducer.class);
			
			for (Path addPath : requestedPaths) {				
				if(addPath.toString().contains(STLD_FILE_TYPE)) {
					MultipleInputs.addInputPath(job, addPath, org.apache.hadoop.mapreduce.lib.input.TextInputFormat.class,
													com.mcd.gdw.daas.mapreduce.NextGenEmixStldMapper.class);
				} else if(addPath.toString().contains(MENU_ITEM_FILE_TYPE)) {
					MultipleInputs.addInputPath(job, addPath, org.apache.hadoop.mapreduce.lib.input.TextInputFormat.class, 
													com.mcd.gdw.daas.mapreduce.NextGenEmixMenuItemMapper.class);
				}					
			}
			...
			MultipleOutputs.addNamedOutput(job, GLMA_Dly_Sls, TextOutputFormat.class, NullWritable.class, Text.class);
	        MultipleOutputs.addNamedOutput(job, GLMA_Hrly_Sls, TextOutputFormat.class, NullWritable.class, Text.class);
	        MultipleOutputs.addNamedOutput(job, GLMA_Dly_Pmix, TextOutputFormat.class, NullWritable.class, Text.class);
	        ...
	        /* Testing 
	        MultipleOutputs.addNamedOutput(job, HAVI_Dly_Sls, TextOutputFormat.class, NullWritable.class, Text.class);
	        MultipleOutputs.addNamedOutput(job, HAVI_Hrly_Sls, TextOutputFormat.class, NullWritable.class, Text.class);
	        MultipleOutputs.addNamedOutput(job, HAVI_Dly_Pmix, TextOutputFormat.class, NullWritable.class, Text.class);
	        MultipleOutputs.addNamedOutput(job, "reducerpmixtest", TextOutputFormat.class, NullWritable.class, Text.class);
	        MultipleOutputs.addNamedOutput(job, "mapperpmixtest", TextOutputFormat.class, NullWritable.class, Text.class);
	        MultipleOutputs.addNamedOutput(job, "menuitems", TextOutputFormat.class, NullWritable.class, Text.class);
	        */
	        ...
	        if (jobStatus == 0) {
	            ...
	            for (int fileCounter = 0; fileCounter < fs.length; fileCounter++) {
	              ...
	              if (fs[fileCounter].getPath().getName().contains(GLMA_Dly_Sls)) {
	            	  ...
	            	  outFileName = "GLMA_Dly_Sls."+timeStamp+fileCounter+".psv";
	            	  
	            	  fileSystem.rename(fs[fileCounter].getPath(), new Path(outputPath + Path.SEPARATOR + outFileName));
					  abac.insertExecutionTargetFile(jobId, fileCounter + 1, outFileName,"NextGen EMIX - GLMA DAILY SALES", 
							  						"Pipe Separated File", recordCount, storeCount);
	              }  else if (fs[fileCounter].getPath().getName().contains(GLMA_Hrly_Sls)) {
	            	  stats = outFileStatsMap.get(fs[fileCounter].getPath().getName()).split(REGEX_PIPE_DELIMITER);
	            	  recordCount = Integer.parseInt(stats[0]);
	            	  storeCount = Integer.parseInt(stats[1]);
	            	  outFileName = "GLMA_Dy_Tm_Seg_Sls."+timeStamp+fileCounter+".psv";
	            	  
	            	  fileSystem.rename(fs[fileCounter].getPath(), new Path(outputPath + Path.SEPARATOR + outFileName));
					  abac.insertExecutionTargetFile(jobId, fileCounter + 1, outFileName, "NextGen EMIX - GLMA HOURLY SALES",
							  						 "Pipe Separated File", recordCount, storeCount);
	              } else if (fs[fileCounter].getPath().getName().contains(GLMA_Dly_Pmix)) {
	            	  stats = outFileStatsMap.get(fs[fileCounter].getPath().getName()).split(REGEX_PIPE_DELIMITER);
	            	  recordCount = Integer.parseInt(stats[0]);
	            	  storeCount = Integer.parseInt(stats[1]);
	            	  outFileName = "GLMA_Dly_Pmix."+timeStamp+fileCounter+".psv";
	            	  
	            	  fileSystem.rename(fs[fileCounter].getPath(), new Path(outputPath + Path.SEPARATOR + outFileName));
					  abac.insertExecutionTargetFile(jobId, fileCounter + 1, outFileName, "NextGen EMIX - GLMA DAILY PMIX",
							  						 "Pipe Separated File", recordCount, storeCount);
	              } 
	              /*   else if (fs[fileCounter].getPath().getName().contains(HAVI_Dly_Sls)) {
	            	  fileSystem.rename(fs[fileCounter].getPath(), new Path(outputPath+"/HAVI_Dly_Sls."+timeStamp+fileCounter+".psv"));
	            	  recordsandStoresInOutputFile = totalLinesinOutPutFile(new Path(outputPath + "/HAVI_Dly_Sls."+timeStamp+fileCounter+".psv"), fileSystem, 1); 
					  abac.insertExecutionTargetFile(jobId, fileCounter + 1, "HAVI_Dly_Sls."+timeStamp+fileCounter, 
							  						"NextGen EMIX - HAVI DAILY SALES", "Pipe Separated File", recordsandStoresInOutputFile[0], recordsandStoresInOutputFile[1]);
	              } else if (fs[fileCounter].getPath().getName().contains(HAVI_Hrly_Sls)) {
	            	  fileSystem.rename(fs[fileCounter].getPath(), new Path(outputPath+"/HAVI_Dy_Tm_Seg_Sls."+timeStamp+fileCounter+".psv"));
	            	  recordsandStoresInOutputFile = totalLinesinOutPutFile(new Path(outputPath+"/HAVI_Dy_Tm_Seg_Sls."+timeStamp+fileCounter+".psv"), fileSystem, 1); 
					  abac.insertExecutionTargetFile(jobId, fileCounter + 1, "HAVI_Dy_Tm_Seg_Sls."+timeStamp+fileCounter, 
							  						"NextGen EMIX - HAVI HOURLY SALES", "Pipe Separated File", recordsandStoresInOutputFile[0], recordsandStoresInOutputFile[1]);
	              }  else if (fs[fileCounter].getPath().getName().contains(HAVI_Dly_Pmix)) {
	            	  fileSystem.rename(fs[fileCounter].getPath(), new Path(outputPath+"/HAVI_Dly_Pmix."+timeStamp+fileCounter+".psv"));
	            	  recordsandStoresInOutputFile = totalLinesinOutPutFile(new Path(outputPath+"/HAVI_Dly_Pmix."+timeStamp+fileCounter+".psv"), fileSystem, 1);
	            	  abac.insertExecutionTargetFile(jobId, fileCounter + 1, "HAVI_Dly_Pmix."+timeStamp+fileCounter, 
	            			  						"NextGen EMIX - HAVI DAILY PMIX", "Pipe Separated File", recordsandStoresInOutputFile[0], recordsandStoresInOutputFile[1]);
	              } */
	 ...
	 * @return
	 */
	private ArrayList<Path> getVaildFilePaths(DaaSConfig daasConfig, FileSystem fileSystem, String fileType, 
			...
			try {
				...
				LOGGER.log(Level.INFO, "Error occured in NextGenEmixDriver.getVaildFilePaths:");					
				LOGGER.log(Level.INFO,ex.toString());
				LOGGER.log(Level.INFO, ex.getMessage(),ex);
				System.exit(1);
			}
	...
	private ArrayList<Path> getVaildFilePaths(DaaSConfig daasConfig, FileSystem fileSystem, 
		...
		Path listPath = new Path(daasConfig.hdfsRoot() + Path.SEPARATOR	+ daasConfig.hdfsWorkSubDir() + 
									Path.SEPARATOR + daasConfig.fileSubDir() + Path.SEPARATOR + inputDir);
	...
	/**
	 * Method to process input from any custom location. All files in the path passed using -i (STLD) and -m (MenuItem)
	...
	/**
	 * To add record and store counts of output files to a map
	 * @param fs
	 * @throws IOException
	 */
        ...
        for( int fileCounter = 0; fileCounter < fs.length; fileCounter++) {
        	if(fs[fileCounter].getPath().getName().contains(Output_File_Stats)) {
        		...
        		while ( (line = br.readLine() )!= null) {
    		        ...
    		        if (stats.length == 3) {
    		        	outFileStatsMap.put(stats[0].trim(), stats[1].trim()+DaaSConstants.PIPE_DELIMITER+stats[2].trim());
    		        }   
    		    }
        	}
        }
