...
import com.mcd.gdw.daas.mapreduce.NewTLDDataHubExtraFieldsReducer;
import com.mcd.gdw.daas.mapreduce.TLDDataHubReducerHCat;
import com.mcd.gdw.daas.mapreduce.TLDDataHubReducerMultiOut;
import com.mcd.gdw.daas.mapreduce.TLDNewDataHubExtraFieldsMapper;
...
public class GenerateDataHubFormatExtraFields extends Configured implements Tool {
	...
	private static final String JOB_GRP_DESC = "Generate TLD New DataHub addnl Fields";
...
//	FsPermission newFilePremission;
	...
	public static void main(String[] args) throws Exception {
		...
		int retval = ToolRunner.run(hdfsConfig,new GenerateDataHubFormatExtraFields(), args);
...
//					businessdatewithhexdelimiter = businessdatewithoutdelimiter.substring(0,4)+"RxD045"+businessdatewithoutdelimiter.substring(4,6)+"RxD045"+businessdatewithoutdelimiter.substring(6, 8);;
				...
				if("840".equalsIgnoreCase(terr_cd)){
					//comment this if you want to do resting for 1 file
...
//			FileInputFormat.addInputPath(job, new Path("/daas/gold/np_xml/STLD/840/20150105/STLD~840~20150105~0000679.gz"));
			...
			FileInputFormat.addInputPath(job, new Path("/daas/gold/np_xml/STLD/840/20150601/STLD~840~20150601~0000096.gz"));
			FileInputFormat.addInputPath(job, new Path("/daas/gold/np_xml/DetailedSOS/840/20150601/DetailedSOS~840~20150601~0000096.gz"));
...
//			Path locationList = new Path(daasConfig.hdfsRoot() + Path.SEPARATOR + daasConfig.hdfsLandingZoneSubDir() + Path.SEPARATOR + "cache" + Path.SEPARATOR + "offers_include_list.txt");
			...
			job.setJarByClass(GenerateDataHubFormatExtraFields.class);
			job.setMapperClass(TLDNewDataHubExtraFieldsMapper.class);
			job.setReducerClass(NewTLDDataHubExtraFieldsReducer.class);
