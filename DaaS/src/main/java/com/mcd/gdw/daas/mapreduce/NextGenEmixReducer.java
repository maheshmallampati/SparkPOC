 ...
 *
 * NextGenEmixReducer class to process pmix data from NextGenEmixStldMapper and MenuItem information from NextGenEmixMenuItemMapper
 */
public class NextGenEmixReducer extends Reducer<Text, Text, Text, Text> {
	...
	private PmixData pmixOutObj = null;
	...
	private int gdwPmixRecordCount;
	private int gdwPmixUniqStoreCount;
	private int[] recordAndStoreCount = new int[2];
	
	@Override
	public void setup(Context context) throws IOException, InterruptedException {
		multipleOutputs = new MultipleOutputs<Text, Text>(context);
		toysItemCodes = NextGenEmixUtil.populateToysItemCodes(START_TOY_CD, END_TOY_CD);
		keyStrBuf = new StringBuffer();
		
		gdwPmixRecordCount = 0;
		gdwPmixUniqStoreCount = 0;
		
		Date date = new Date();
		SimpleDateFormat gdwDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.S"); // Change to constant
		SimpleDateFormat haviDateFormat = new SimpleDateFormat("MM/dd/yyyy HH:mm:ss"); // Change to constant
	...
	public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
		...
		for (Text value : values) {
			...
			mapOutputArray = valueStr.split(NextGenEmixDriver.REGEX_PIPE_DELIMITER);
			if (mapOutputArray[0].equals(NextGenEmixDriver.STLD_TAG_INDEX)) {
			...
			} else if (mapOutputArray[0].equals(NextGenEmixDriver.MENU_ITEM_TAG_INDEX)) {
			...
			if (toyPrices.size() > 0) {
				keyStrBuf.setLength(0);
				keyStrBuf.append(dataKey).append(DaaSConstants.PIPE_DELIMITER).append(NextGenEmixDriver.HAPPY_MEAL_TOY_CD);
		...
		for (String eachPmixKey : pmixDataMapIn.keySet()) {
			...
			itemPriceKeyArray = eachPmixKey.split(NextGenEmixDriver.REGEX_PIPE_DELIMITER);
			...
			pmixInArray = pmixIn.split(NextGenEmixDriver.REGEX_PIPE_DELIMITER);
			
			if (pmixInArray[12].equals("999.99")) {
				
				lookupPrice = itemPriceLookup.containsKey(itemPriceKey) ? itemPriceLookup.get(itemPriceKey) : NextGenEmixDriver.DECIMAL_ZERO_STR;
				pmixKeyTokens = eachPmixKey.split(NextGenEmixDriver.REGEX_PIPE_DELIMITER);
				...
				if (pmixInArray[17].equals("Y")) {
					...
					for (String eachCmptWithQty : comboComponents) {
						...
						if (componentDetails.length == 2) {
							...
							System.out.println(">> Components Issue :  " + eachCmptWithQty);
						}
						...
						menuItemPrice = itemPriceLookup.containsKey(lookupKey) ? new BigDecimal(itemPriceLookup.get(lookupKey)) : NextGenEmixDriver.DECIMAL_ZERO;
					...
					if (pmixDataObj.getComboFullPrice().compareTo(NextGenEmixDriver.DECIMAL_ZERO) == 0) {
						pmixDataObj.setComboUpDownAmt(NextGenEmixDriver.DECIMAL_ZERO);
					} else {
		...
		String[] pmixTokens = null;
		for (String eachPmixKey : pmixDataMap.keySet()){
			pmixDataObj = pmixDataMap.get(eachPmixKey);
			pmixTokens = eachPmixKey.split(NextGenEmixDriver.REGEX_PIPE_DELIMITER);
			
			keyStrBuf.setLength(0);
			keyStrBuf.append(pmixTokens[0]).append(DaaSConstants.PIPE_DELIMITER).append(pmixTokens[1]).append(DaaSConstants.PIPE_DELIMITER)
					 .append(pmixTokens[2]).append(DaaSConstants.PIPE_DELIMITER).append(pmixTokens[3]);
			newPmixKey = keyStrBuf.toString();
			
			pmixOutObj = finalPmixOutputMap.containsKey(newPmixKey) ? finalPmixOutputMap.get(newPmixKey) : new PmixData();
			
			pmixOutObj.setTerrCd(pmixDataObj.getTerrCd());
			pmixOutObj.setStoreId(pmixDataObj.getStoreId());
			pmixOutObj.setGdwBusinessDate(pmixDataObj.getGdwBusinessDate());
			pmixOutObj.setItemCode(pmixDataObj.getItemCode());
			pmixOutObj.setConsPrice(pmixDataObj.getConsPrice());
			
			if (pmixDataObj.getDlyPmixPrice().compareTo(NextGenEmixDriver.DECIMAL_ZERO) > 0) {
				pmixOutObj.setMenuItemCount(pmixOutObj.getMenuItemCount() + pmixDataObj.getLvl0PmixQty());
			} 
			
			pmixOutObj.setDlyPmixPrice(pmixOutObj.getDlyPmixPrice().add(pmixDataObj.getDlyPmixPrice()));
			pmixOutObj.setTotalDlyPmixPrice(pmixOutObj.getTotalDlyPmixPrice().add(pmixDataObj.getTotalDlyPmixPrice()));
			
			pmixOutObj.setComboUpDownAmt(pmixOutObj.getComboUpDownAmt().add(pmixDataObj.getComboUpDownAmt()));
			
			pmixOutObj.setUnitsSold(pmixOutObj.getUnitsSold() + pmixDataObj.getUnitsSold());
			pmixOutObj.setComboQty(pmixOutObj.getComboQty() + pmixDataObj.getComboQty());
			pmixOutObj.setTotalQty(pmixOutObj.getTotalQty() + pmixDataObj.getTotalQty());
			
			pmixOutObj.setPromoQty(pmixOutObj.getPromoQty() + pmixDataObj.getPromoQty());
			pmixOutObj.setPromoComboQty(pmixOutObj.getPromoComboQty() + pmixDataObj.getPromoComboQty());
			pmixOutObj.setPromoTotalQty(pmixOutObj.getPromoTotalQty() + pmixDataObj.getPromoTotalQty());
			
			pmixOutObj.setStoreType(pmixDataObj.getStoreType());
			pmixOutObj.setHaviBusinessDate(pmixDataObj.getHaviBusinessDate());
			pmixOutObj.setHaviTimeKey(pmixDataObj.getHaviTimeKey());
			
			finalPmixOutputMap.put(newPmixKey, pmixOutObj);
		}		
		
		recordAndStoreCount = NextGenEmixUtil.writeGDWPmixtoMultipleOutput(multipleOutputs, finalPmixOutputMap, gdwTimeStamp, haviTimeStamp);
		gdwPmixRecordCount += recordAndStoreCount[0];
		gdwPmixUniqStoreCount += recordAndStoreCount[1];
		...
		for (String eachPmixKey : pmixDataMap.keySet()) {
			...
			if (pmixOutData.getTotalQty() > 0) {
				multipleOutputs.write("reducerpmixtest", NullWritable.get(), 
						new Text(new StringBuffer(pmixOutData.getTerrCd()).append(DaaSConstants.PIPE_DELIMITER)
								...
								.append(gdwTimeStamp).append(DaaSConstants.PIPE_DELIMITER)
								.append(pmixOutData.getComboFullPrice()).append(DaaSConstants.PIPE_DELIMITER)
								.append(pmixOutData.getOrderKey()).toString()));
			}
	...
	public void cleanup(Context context) throws IOException,InterruptedException {
		...
		multipleOutputs.write(NextGenEmixDriver.Output_File_Stats, NullWritable.get(), 
				new Text(NextGenEmixDriver.GLMA_Dly_Pmix+"-r-"+partitionID + DaaSConstants.PIPE_DELIMITER + gdwPmixRecordCount 
								+ DaaSConstants.PIPE_DELIMITER + gdwPmixUniqStoreCount));
		try {
