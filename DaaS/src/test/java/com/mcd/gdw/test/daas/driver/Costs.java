...
import com.mcd.gdw.test.daas.mapreduce.CostsMapper;
import com.mcd.gdw.test.daas.mapreduce.CostsReducer;

public class Costs extends Configured implements Tool {
	...
	public static void main(String[] args) throws Exception {
		...
		int retval = ToolRunner.run(hdfsConfig,new Costs(), args);
	...
	public int run(String[] args) throws Exception {
		...
		if ( daasConfig.configValid() ) {
			...
			baseOutputPath = new Path("/daastest" + Path.SEPARATOR + daasConfig.hdfsWorkSubDir() + Path.SEPARATOR + "Costs");
	...
	private void runJob(DaaSConfig daasConfig
		...
		try {
			...
			requestedPaths.add(new Path("/daastest" + Path.SEPARATOR + daasConfig.hdfsFinalSubDir() + Path.SEPARATOR + "FPCosts" + Path.SEPARATOR + "840" + Path.SEPARATOR + "201203"));

			job = Job.getInstance(hdfsConfig, "Costs");
			...
			job.setJarByClass(Costs.class);
			job.setMapperClass(CostsMapper.class);
			job.setReducerClass(CostsReducer.class);
