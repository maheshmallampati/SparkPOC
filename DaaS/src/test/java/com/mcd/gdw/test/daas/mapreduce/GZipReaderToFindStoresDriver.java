...
public class GZipReaderToFindStoresDriver extends Configured implements Tool {

	public static void main(String[] argsAll) throws Exception{
		Configuration conf = new Configuration();
		
		int ret = ToolRunner.run(conf, new GZipReaderToFindStoresDriver(),argsAll);
...
//		getConf().setInt(NLineInputFormat.LINES_PER_MAP,10);
		...
		Job job = new Job(getConf(),"FindStoresByFileTypefromGL");
		
		job.setJarByClass(GZipReaderToFindStoresDriver.class);
		
		job.setMapperClass(GZipReaderToFindStoresMapper.class);
...
//			if(fileNameorg.contains("STLD~840~20141028~0001252.gz"))
		...
		while(it.hasNext()){
			...
			MultipleOutputs.addNamedOutput(job, "StoreIdsByType"+DaaSConstants.SPLCHARTILDE_DELIMITER+moutfilename, TextOutputFormat.class, NullWritable.class, Text.class);
		}
